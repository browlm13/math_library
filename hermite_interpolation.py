...
def hermite_coeffs(xs, ys, dys, output=True):
	...
	# modify ys and xs so that they repeate values: [y0,y1,y2] -> [y0,y0,y1,y1,y2,y2]
...
if __name__ == "__main__":
	...
	# log to screen
	logger.info("\n\nEvaluating the interpolating polynomial at x=%s: \n\n\tPn(%s) = %s.\n\nExiting Newton Divided-Difference.\n" % (x, x, pnx))

	#import matplotlib.pyplot as plt
	#axs = np.linspace(-10, 10)
	#ays = np.arctan(2*axs**2)
	#cys = [evaluate_newton_polynomial(ax,xs,bs) for ax in axs]
	#plt.plot(axs, ays, color='k')
	#plt.plot(axs, cys, color='r')
	#plt.show()
	...
	# compute coefficients
	#cap = int((len(xs) + 1)/2 -1)
	#bs = hermite_coeffs(xs[:cap],ys[:cap],dys[:cap])
	bs = hermite_coeffs(xs,ys,dys)

	# evaluate interpolating polynomial at x

	# NOTE: modify xs
	mod_xs = np.repeat(xs, 2)
	pnx = evaluate_newton_polynomial(x,mod_xs,bs)
	...
	# plot hermite interpolation vs. actual
	axs = np.linspace(-3, 3, 100)
	ays = np.arctan(2*axs**2)
	cys = [evaluate_newton_polynomial(ax,mod_xs,bs) for ax in axs]
	plt.plot(axs, ays, color='k')
	plt.plot(axs, cys, color='r')
	plt.show()

	# [TODO]: plot hermite interpolation derivative vs. actual derivative
