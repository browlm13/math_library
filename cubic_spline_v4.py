...
__filename__ = "cubic_spline_v4.py"
...
def cubic_spline_evaluate(t, y, z, xs):
	...
	for x in xs:
		...
		for i in range(1, n):
			if x < t[i]: 
				results.append( S(i-1,x) )
		results.append( S(n-1, x) )
...
if __name__ == '__main__':
	...
	z2 = cubic_spline_coefficients_bc(t,y,start_bc=(1,alpha), end_bc=(2,beta))

	interpolation_ys = cubic_spline_evaluate(t,y,z,t)
	interpolation_ys2 = cubic_spline_evaluate(t,y,z2,t)

	xs = np.linspace(-L, L, 400)     # evaluation points
	yhats = cubic_spline_evaluate(t,y,z,xs)
	yhats2 = cubic_spline_evaluate(t,y,z2,xs)
	...
	axarr[0].scatter(t,interpolation_ys2,s=5,color='b', zorder=2)

	fplot = axarr[0].plot(xs, f(xs), 'r-', label=ftitle)
	splot = axarr[0].plot(xs, yhats, 'g--', label=stitle)
	s2plot = axarr[0].plot(xs, yhats2, 'y', label=s2title)
