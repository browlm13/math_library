    	...
    	==========================
    		...
    		------------------------------------------------------------------
    		    ...
    		    constructed to interpolate the table data above.
    				...
    				On each of the n subintervals: (-inf, t_1], [t_1, t_2], ..., [t_n-1, +inf), 
	    			...
	    			* S_0, ..., S_n-1 ~ 
					    ...
					    a_i0, ..., a_i3,
					   	...
					   	S_i =  (a_i0)  +(a_i1)*(x)  +(a_i2)*(x^2)  +(a_i3)*(x^3)
		    					...
		    					{  
		    					   S_0( x )   	for x in [-inf, t_1],
		    				       S_1( x )   	for x in [t_1, t_0],

					S( x ) =	   .
							 	   .
								   .

		    					   S_n-1( x ) 	for x in [t_n-1, +inf]  
		    					}
...
def cubic_spline_coefficients(t, y, alpha, beta):
	...
	for i in range(1,n):
		...
		# fill result vector neglecting first and last elements
		r[i] = v(i)
...
def cubic_spline_evaluate(t, y, z, x):
	...
	h = lambda i: t[i+1] - t[i] # h_i = t_i+1 - t_i
